...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<thead>
	  ...
	  <tbody>
              <?php
                $somaTotal = 0;
                $i=0;
                $lst_carinhho = consultar("carrinho c, produtos p "," c.id_produto = p.id_produto and  id_pedido = $idPedido ");
		...
		<td align="center"><input type="Number" name="textfield" id="textfield" value="<?php echo  $carrinho["quantidade"] ?>" class="cont"/></td>
		<td align="center"><h3>R$ <?php echo $carrinho["valor"] ?></h3></td>
		<td align="center">R$ <?php echo $carrinho["valor"] * $carrinho["quantidade"] ?></h3></td>
		</tr>
	  	<?php }?> 
    ...
    <h3 class="total" >
        <form>
            Cep:<input type="text">
            <input type="button" value="Calcular frete" id="btnfrete">
        </form>
        Frete: <span id="valorfrete">R$ 0,00</span></h3>
    <h3 class="total" id="valortotal">Soma total: R$<?php echo $somaTotal?></h3>
